...
class ReportFinancial(models.AbstractModel):
    _name = 'report.geedy_accounting.report_cash_flow'
            ...
            if report.type == 'accounts':
                ...
                for key, value in res2.items():
                    cash_in_operation = self.env.ref(
                        'geedy_accounting.cash_in_from_operation0')
                    cash_out_operation = self.env.ref(
                        'geedy_accounting.cash_out_operation1')
                    cash_in_financial = self.env.ref(
                        'geedy_accounting.cash_in_financial0')
                    cash_out_financial = self.env.ref(
                        'geedy_accounting.cash_out_financial1')
                    cash_in_investing = self.env.ref(
                        'geedy_accounting.cash_in_investing0')
                    cash_out_investing = self.env.ref(
                        'geedy_accounting.cash_out_investing1')
...
class GeneralLedger(models.AbstractModel):
    _name = 'report.geedy_accounting.cash_flow'
